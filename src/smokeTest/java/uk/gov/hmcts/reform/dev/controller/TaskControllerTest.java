...
public class TaskControllerTest {
    ...
    void createNewTask_savedTheNewTask() {
        ...
        given().contentType(ContentType.JSON).body(task1).when()
            .post("/api/task/createNewTask").then().statusCode(201)
            .body(
            "status",
            equalTo("new")
    ...
    void getAllTask_shouldReturnAllTheSavedTasks() {
        ...
        given().contentType(ContentType.JSON).when().get("/api/task/getAllTasks").then().statusCode(200) // HTTP 200 OK
            .body("$", hasSize(count)) // Expect two tasks in the list
            .body("[0].title", equalTo("Test")).body("[1].title", equalTo("Test2"));
    }
    ...
    void getTaskById_shouldReturnAllTheSavedTasks() {
        ...
        given().queryParam(
                "id",
                task1.getTaskId()
            ).when().get("/api/task/getTaskById").then().log().all().statusCode(200) // HTTP 200 OK
    ...
    void getTaskById_WhenNoMatchingTaskFound() {
        given().queryParam(
            "id",
            2L
        ).when().get("/api/task/getTaskById").then().log().all().statusCode(204); // HTTP 204 No Content
    }
    ...
    void updateTask_BasedOnIdWithStatus() {
        ...
        given().queryParam("id", task1.getTaskId()).queryParam("status", "Inprogress").when().post(
                "/api/task/UpdateTaskStatus").then().statusCode(200) // HTTP 200 OK
            .body("status", equalTo("Inprogress"));
    }

    @Test
    void updateTask_shouldReturnUpdatedTask_whenFound() {
        given().queryParam("id", 4L).queryParam(
            "status",
            "Inprogress"
        ).when().post("/api/task/UpdateTaskStatus").then().statusCode(204);// HTTP 500 OK;
    }
    ...
    void deleteTask_shouldReturnNoContent_whenDeleted() {
        ...
        given().when().delete(
            ...
            task1.getTaskId()
        ).then().statusCode(200); // HTTP 204 No Content
